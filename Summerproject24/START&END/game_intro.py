...
# Define functions to load and scale images
def load_image(path):
...
game_logo = load_image("game_logo.png")
start_button = load_image("start_button.png")
options_button = load_image("options_button.png")
back_button = load_image("back_button.png")
brightness_button = load_image("brightness_button.png")
...
def draw_second_page_logo():
    ...
    screen.blit(pygame.transform.scale(dim_button, (scaled_dim_button_width, scaled_dim_button_height)), (button_x_dim, button_y_dim))  # Display the dim button
        ...
        if event.type == pygame.QUIT:
            ...
            if not second_page_active:
                ...
                if button_x_back <= mouse_pos[0] <= button_x_back + scaled_back_button_width and button_y_back <= mouse_pos[1] <= button_y_back + scaled_back_button_height:
                    ...
                    # Handle brightness button click event here
                    print("Brightness Button Clicked!")
                    brightness_level = min(1.2, brightness_level + 0.01)  # Increase brightness level, max 2.0
                    ...
                    # Handle dim button click event here
                    print("Dim Button Clicked!")
                    brightness_level = max(0.0, brightness_level - 0.01)  # Decrease brightness level, min 0.0
